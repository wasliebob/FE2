...
double d2, float f) {
	...
	TileEntityProducer block = ((TileEntityProducer) tileentity);
         ...
         if (((block.getStackInSlot(0).getItem() instanceof ItemBlock)) && (block.getStackInSlot(0).itemID < Block.blocksList.length)) {
         ...
         ist.stackSize = 1;
         entityitem = new EntityItem(block.worldObj, 0.0D, 0.0D, 0.0D, ist);
